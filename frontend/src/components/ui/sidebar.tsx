...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  // This is a simple implementation, you can use a hook for better support
  React.useEffect(() => {
  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      // This is where you'd write to a cookie or localStorage
  ...
  ({ className, ...props }, ref) => {
    return (
      <div
        ref={ref}
        data-sidebar="header"
        className={cn("flex h-[88px] shrink-0 items-center justify-center border-b border-sidebar-border", className)}
        {...props}
      />
    )
  }
...
>(({ className, showIcon = false, ...props }, ref) => {
  ...
  return <div ref={ref} className={cn("rounded-md h-8", className)} {...props} />
})
...
>(({ asChild = false, size = "md", isActive, className, ...props }, ref) => {
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 text-sidebar-foreground outline-none ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-primary data-[active=true]:text-primary-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 [&>svg]:text-sidebar-foreground/50",
        "group-data-[collapsible=icon]:hidden",
        size === "sm" && "text-xs",
        size === "md" && "text-sm",
        size === "lg" && "text-sm",
